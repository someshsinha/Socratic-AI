  ...
  return (
    ...
    >
      <div className="max-w-[1240px] mx-auto px-6 flex items-center justify-between gap-6 py-2">
        ...
        {/* Desktop nav with square borders on active/hover */}
          ...
          {isLoading ? (
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="px-4 py-2 border text-sm font-bold text-white hover:-translate-y-px transition-transform hidden sm:inline-block"
                style={{ borderColor: '#111827', background: '#111827' }}
              >
                My Courses →
              </button>
              {user?.picture && (
                <img src={user.picture} alt={user.name} className="h-8 w-8 border border-[#d8d3c7] object-cover" />
              )}
              <span className="hidden lg:inline text-sm" style={{ color: '#5f6673' }}>
                {user?.name || user?.nickname}
              </span>
              <button
                onClick={() => logout({ logoutParams: { returnTo: window.location.origin } })}
                className="px-3 py-1.5 border text-xs font-bold hover:-translate-y-px transition-transform"
                style={{ borderColor: '#d8d3c7', color: '#111827', background: 'transparent' }}
              >
                Sign Out
              </button>
            </div>
